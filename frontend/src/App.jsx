...
function Tasks({ tasks, onCreate, onEdit, onDelete, onStatus }) {
...
function App() {
  ...
  // The loader synchronizes several API-backed state values after authentication.
  ...
  const logout = async () => { try { await api('/auth/logout', { method: 'POST' }) } catch { localStorage.removeItem('task-university-token') } finally { localStorage.removeItem('task-university-token'); setUser(null) } }
