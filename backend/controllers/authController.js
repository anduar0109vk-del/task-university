...
async function login(req, res) {
    ...
    await pool.execute('INSERT INTO auditoria (usuario_id, accion, tabla_afectada, registro_id, detalles, direccion_ip) VALUES (?,?,?,?,?,?)',
        [user.id, 'LOGIN', 'usuarios', user.id, `Inicio de sesión desde ${req.get('user-agent') || 'cliente desconocido'}`, req.ip]);
    const { contrasena_hash, ...safe } = user;
    res.json({ token, usuario: safe, user: safe });
}
async function logout(req, res) {
    revokeToken(req.token);
    try { await pool.execute('INSERT INTO auditoria (usuario_id, accion, tabla_afectada, registro_id, detalles, direccion_ip) VALUES (?,?,?,?,?,?)', [req.user.id, 'LOGOUT', 'usuarios', req.user.id, 'Cierre de sesión', req.ip]); } catch (_) {}
